...
class UIManager:
    ...
    def create_driver_info(self):
        ...
        driver_frame = ctk.CTkFrame(self.right_panel, fg_color="#F5F5F5", border_width=1, border_color="#CCCCCC")
        ...
        details_frame = ctk.CTkFrame(driver_frame, fg_color="transparent")
        ...
        self.vehicle_value = ctk.CTkLabel(
            right_col,
            text="N/A",
            font=ctk.CTkFont(size=10, weight="bold"),
            text_color="#000000"
        )
        self.vehicle_value.pack(anchor="w", pady=(5, 0))
    ...
    def update_driver_info(self, vehicle_type):
        """Update the displayed driver information based on selected vehicle type"""
        driver_info = self.driver_manager.get_driver_info(vehicle_type)
        if driver_info:
            self.driver_name_value.configure(text=driver_info['driver_name'])
            self.plate_value.configure(text=driver_info['plate_no'])
            self.vehicle_value.configure(text=driver_info['vehicle_name'])
            self.contact_value.configure(text=driver_info['contact_no'])
        else:
            self.driver_name_value.configure(text="Not assigned")
            self.plate_value.configure(text="N/A")
            self.vehicle_value.configure(text="N/A")
