...
from driver_management import DriverManager
from Book_history import BookingHistory
from CALCULATIONS.eta_calculator import ETACalculator
...
class MapWithDistanceRideBookingApp:
    ...
    def setup_components(self):
        ...
        self.driver_manager = DriverManager("driver.csv")
    ...
    def show_booking_confirmation_with_cancellation(self, booking_details):
        ...
        if eta_info and eta_info['eta_minutes'] > 0:
            ...
            if "traffic_condition" in eta_info:
                booking_message += f"🚦 Traffic: {eta_info['traffic_condition']}\n"
        ...
        try:
            ...
            payment_mode = self.ui_manager.get_payment_mode() or "Cash"
            price = self.ui_manager.get_selected_vehicle_price() or "₱ 0.00"
        
            # Add booking record to history
            ...
            print(f"✅ Added booking {booking_id} to history")
        except Exception as e:
    ...
    def keep_ride_from_window(self):
        ...
        try:
            ...
            if self.current_booking:
                ...
                # *** REMOVED: Don't release driver here - keep them busy until trip completes ***
                # released_driver = self.driver_manager.release_driver_to_available(booking_id)
    ...
    def show_trip_duration_popup(self):
        ...
        try:
            ...
            # We need to store current_booking info before it gets set to None
            if not hasattr(self, '_last_booking_info') or not self._last_booking_info:
                # If no stored booking info, try to get from current_booking
                if self.current_booking:
            ...
            vehicle_type = booking_info.get('vehicle_type', 'Car4Seater')
            vehicle = self.get_vehicle_by_type(vehicle_type)
        
            if not vehicle:
                messagebox.showwarning("Error ⚠️", "Vehicle information not available")
                return
            ...
            # MODIFICATION 1: Remove grab_set() to allow minimizing
            # self.trip_dialog.grab_set()  # Comment out or remove this line
            
            # MODIFICATION 2: Don't make dialog always on top
            # self.trip_dialog.attributes('-topmost', True)  # Comment out or remove this line
            ...
            # MODIFICATION 3: Add minimize/maximize functionality
            ...
            # MODIFICATION 4: Add header frame with minimize button
            header_frame = ctk.CTkFrame(main_frame, fg_color="transparent")
            header_frame.pack(fill="x", pady=(0, 10))
            
            # Title (moved to header frame)
            title_label = ctk.CTkLabel(
                header_frame,
            ...
            # Traffic info if available (moved to content frame)
            if eta_info.get('traffic_condition'):
                traffic_label = ctk.CTkLabel(
                    self.trip_content_frame,
                    text=f"🚦 {eta_info['traffic_condition']}",
                    font=ctk.CTkFont(size=12),
                    text_color="#FF9800"
                )
                traffic_label.pack(pady=(0, 15))
            ...
            # MODIFICATION 5: Change dialog close behavior to allow minimizing
            ...
            messagebox.showerror("Error ❌", "Failed to show trip duration popup")
    ...
    def show_eta_comparison(self):
        ...
        for vehicle_type in vehicle_types:
            ...
            if vehicle:
                ...
                if "traffic_condition" in eta_info:
                    comparison_text += f"   🚦 {eta_info['traffic_condition']}\n"
                
                comparison_text += "\n"
        
        messagebox.showinfo("ETA Comparison 📊", comparison_text)
    
    def get_distance_and_eta_info(lat1, lng1, lat2, lng2, vehicle_type="Car4Seater"):
        from CALCULATIONS.vehicle import Car4Seater, Car6Seater, Minivan, Van, Motorcycle
        ...
        vehicle_map = {
            ...
            "Van": Van("Sample Van", "Van"),
            "Motorcycle": Motorcycle("Sample Motorcycle")
        }
    
        vehicle = vehicle_map.get(vehicle_type, Car4Seater("Default Car"))
        ...
        return {
            ...
            "traffic_condition": eta_info.get('traffic_condition', 'Unknown'),
            ...
            if pickup_pos and dropoff_pos:
                ...
                if selected_vehicle:
                    ...
                    if vehicle:
                        ...
                        if "traffic_condition" in eta_info:
                            eta_text += f"\n🚦 {eta_info['traffic_condition']}"
                            
                            # Show delay if significant
                            if eta_info.get("delay_minutes", 0) > 1:
                                delay_text = ETACalculator.format_eta_time(eta_info["delay_minutes"])
                                eta_text += f" (+{delay_text})"
    ...
    def get_vehicle_by_type(self, vehicle_type):
        """
        Get a vehicle object by type for ETA calculation
        You'll need to create vehicle instances or import from your vehicle module
        """
        from CALCULATIONS.vehicle import Car4Seater, Car6Seater, Minivan, Van, Motorcycle
